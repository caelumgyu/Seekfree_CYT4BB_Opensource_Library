...
int16 x_zero = 0, y_zero = 0, z_zero = 0;
...
void imu660rc_calculate_offset()
{
    system_delay_ms(100);

    int16 x_sum = 0, y_sum = 0, z_sum = 0;
    uint16 count = 0;

    while (count < 500)
    {
        imu660rc_get_gyro();
        if (imu660rc_gyro_x > 50 || imu660rc_gyro_y > 50 || imu660rc_gyro_z > 50)
            continue;
        x_sum += imu660rc_gyro_x;
        y_sum += imu660rc_gyro_y;
        z_sum += imu660rc_gyro_z;

        count++;
        system_delay_ms(1);
    }
    x_zero = (x_sum / count);
    y_zero = (y_sum / count) + 1;
    z_zero = (z_sum / count) + 1;
}
